    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-8 max-w-lg w-full text-gray-900">
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-2xl font-black">🚲 Cycle Shop</h2>
            <p className="text-gray-500 text-sm">Sale Bill</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600">✕</button>
        </div>
          <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
            <div><p className="text-gray-500">Bill No</p><p className="font-bold">{order.id}</p></div>
            <div><p className="text-gray-500">Date</p><p className="font-bold">{order.date ? new Date(order.date).toLocaleDateString() : 'N/A'}</p></div>
            <div><p className="text-gray-500">Buyer Name</p><p className="font-bold">{order.customer || order.customerName || 'Guest'}</p></div>
            <div><p className="text-gray-500">How Paid</p><p className="font-bold">{order.payment || order.paymentMethod || 'N/A'}</p></div>
          </div>
        ...
        <div className="border-t border-gray-200 pt-4 space-y-1 text-sm">
          ...
          <div className="flex justify-between font-black text-lg pt-2 border-t border-gray-200"><span>Grand Total</span><span>₹{(order.total || 0).toLocaleString()}</span></div>
        </div>
        <button onClick={() => window.print()} className="mt-5 w-full bg-gray-900 text-white py-2.5 rounded-xl font-semibold text-sm">Print Bill</button>
      </div>
    </div>
  );
}
...
export default function Orders() {
  ...
  const orderCategories = ['All Categories', ...new Set(orders.flatMap(o => (o.items || []).map(i => i.category)).filter(Boolean))];
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">{orders.length} total sales • {filtered.length} shown</p>
        </div>
      </div>
                ...
                <tr key={`${order.id}-${idx}`} className="hover:bg-gray-800/30 transition-all">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex gap-2">
                      <button onClick={() => setSelectedOrder(order)} className="text-xs bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 px-2.5 py-1 rounded-lg">Open</button>
