              ...
              <tr key={c.id} className="hover:bg-gray-800/30 transition-all">
                <td className="px-5 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400 font-bold text-sm">{c.name[0]}</div>
                    <div>
                ...
                <td className="px-5 py-3"><span className="text-sm font-bold text-white">{c.orders}</span></td>
                <td className="px-5 py-3"><span className="text-sm font-bold text-emerald-400">₹{c.totalSpent.toLocaleString()}</span></td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 max-w-md w-full">
            ...
            <div className="flex items-center gap-4 mb-5">
              <div className="w-16 h-16 rounded-2xl bg-amber-500/20 flex items-center justify-center text-amber-400 font-black text-2xl">{selected.name[0]}</div>
              <div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              <div className="bg-gray-800 rounded-xl p-3 text-center">
                <p className="text-xl font-black text-white">{selected.orders}</p>
                <p className="text-xs text-gray-500">Orders</p>
              </div>
              <div className="bg-gray-800 rounded-xl p-3 text-center">
                <p className="text-xl font-black text-emerald-400">₹{(selected.totalSpent/1000).toFixed(0)}k</p>
              ...
              {getCustomerOrders(selected.email).length > 0 ? (
                getCustomerOrders(selected.email).map(o => (
                  <div key={o.id} className="flex justify-between py-1.5 border-b border-gray-700 last:border-0 text-sm">
                    ...
                    <span className="text-gray-300">₹{o.total.toLocaleString()}</span>
