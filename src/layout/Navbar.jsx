...
export default function Navbar() {
  ...
  const navbarBg = "bg-slate-900  border-b border-slate-800 shadow-xl shadow-black/20";
          ...
          <div className="hidden lg:flex items-center gap-2">
            {navItems.map((item) => (
              <Link
                key={item.label}
                to={item.href}
                className={`relative px-4 py-2 text-sm font-black rounded-lg transition-all duration-300 group ${
            ...
            >
              <button className={`px-4 py-2 text-sm font-bold rounded-lg transition-all flex items-center gap-1.5 ${categoriesDropdown ? "text-brand-500 bg-white/5" : "text-white hover:text-brand-400"
                }`}>
            ...
            >
              <button className={`px-4 py-2 text-sm font-bold rounded-lg transition-all flex items-center gap-1.5 ${pagesDropdown ? "text-brand-500 bg-white/5" : "text-white hover:text-brand-400"
                }`}>
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 h-full w-full bg-slate-900 p-6 sm:p-8 z-[80] flex flex-col shadow-2xl overflow-y-auto custom-scrollbar"
            >
              <div className="flex justify-between items-center mb-6 sm:mb-10">
                ...
                <button
                  onClick={() => setOpen(false)}
                  className="p-3 bg-white/5 rounded-2xl text-white hover:text-brand-500 hover:bg-white/10 transition-all border border-white/5"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="mb-8">
                {user ? (
                   <div className="space-y-4">
                      <div className="flex items-center gap-4 p-4 bg-white/5 rounded-[2.5rem] border border-white/5">
                        <div className="w-16 h-16 bg-gradient-to-tr from-brand-600 to-indigo-600 rounded-[2rem] flex items-center justify-center text-white text-2xl font-black shadow-2xl shadow-brand-500/20">
                          {(user.name || 'K')[0].toUpperCase()}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xl font-black text-white truncate">{user.name || 'Karthick R'}</p>
                          <p className="text-xs font-bold text-slate-400 truncate">{user.email || 'karthickrangappan@gmail.com'}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 px-2">
                        {[
                          ...
                          { label: "My Orders", icon: Package, href: "/my-orders" },
                          { label: "My Address", icon: MapPin, href: "/profile?tab=address" },
                          ...(user.role === 'admin' ? [{ label: "Admin Panel", icon: ShieldCheck, href: "/admin", special: true }] : [])
                        ].map((link) => (
                           <Link
                             key={link.label}
                             to={link.href}
                             ...
                             className={`flex flex-col items-center justify-center p-4 rounded-3xl transition-all border ${
                                ...
                                : "bg-white/x5 border-white/5 text-slate-300 hover:text-white"
                             }`}
                           >
                      ...
                      <button 
                         onClick={logout}
                         className="w-full flex items-center justify-center gap-2 p-4 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-2xl font-black uppercase tracking-widest text-xs transition-all border border-red-500/20"
                      >
                         <LogOut size={16} />
                         Sign Out
                      </button>
                   ...
                   <Link
                     to="/login"
                     onClick={() => setOpen(false)}
                     className="flex items-center justify-between p-6 bg-brand-500 rounded-3xl text-white shadow-2xl shadow-brand-500/30 group"
                   >
                     <div>
                        <p className="text-xl font-black">Welcome Back</p>
                        <p className="text-xs font-bold text-white/70">Login to your account</p>
                     </div>
                     <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center group-hover:bg-white group-hover:text-brand-500 transition-all">
                        <ArrowRight size={20} />
                     </div>
                   </Link>
                )}
              ...
              <div className="space-y-2 flex-grow">
                {navItems.map((item) => (
                  <Link
                    ...
                    to={item.href}
                    onClick={() => setOpen(false)}
                    className={`flex justify-between items-center p-4 rounded-2xl transition-all ${location.pathname === item.href
                ...
                <div className="pt-2">
                   <button 
                     ...
                     className="w-full flex items-center justify-between p-4 rounded-2xl text-white font-black hover:bg-white/5 transition-all group"
                   >
                     <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-slate-400 group-hover:text-brand-500 transition-colors">
                           <Info size={18} />
                        </div>
                        <span>Explore Pages</span>
                     </div>
                     <ChevronDown size={18} className={`text-slate-500 transition-transform duration-300 ${mobilePagesOpen ? 'rotate-180' : ''}`} />
                   </button>
                   ...
                   <AnimatePresence>
                     {mobilePagesOpen && (
                       <motion.div 
                         ...
                         className="overflow-hidden bg-white/5 rounded-2xl mt-1 mx-2"
                       >
                         {pageItems.map((item) => (
                           <Link
                             key={item.href}
                             to={item.href}
                             onClick={() => setOpen(false)}
                             className="flex items-center gap-4 p-4 rounded-xl text-slate-300 font-bold hover:bg-white/5 transition-all"
                           >
                ...
                <div className="pt-2">
                   ...
                   >
                     ...
                     <ChevronDown size={18} className={`text-slate-500 transition-transform duration-300 ${mobileCategoriesOpen ? 'rotate-180' : ''}`} />
                   </button>
              ...
              <div className="mt-8 pt-8 border-t border-white/5 text-center">
                 <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600">Premium Cycle Store © 2026</p>
              </div>
            </motion.div>
          </>
        )}
