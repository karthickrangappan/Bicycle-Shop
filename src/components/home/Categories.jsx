...
export default function Categories() {
  ...
  // Filter for active categories and sort by priority
          ...
          {activeCategories.map((category, idx) => (
            ...
            >
              ...
              >
                ...
                {/* Dynamic Gradient Overlay using Firebase Color */}
                <div
                  className="absolute inset-0 transition-opacity duration-500 opacity-80 group-hover:opacity-90"
                  style={{
                    background: `linear-gradient(to top, ${category.color || '#000'} 0%, rgba(15, 23, 42, 0.6) 50%, transparent 100%)`
                  }}
                />
                ...
                <div className="absolute inset-0 p-6 sm:p-8 flex flex-col justify-end text-white">
                  ...
                  <div className="mb-4 h-0 overflow-hidden transform translate-y-6 opacity-0 transition-all duration-500 group-hover:h-auto group-hover:translate-y-0 group-hover:opacity-100">
                    <p className="text-white/80 text-[10px] sm:text-xs font-medium leading-relaxed max-w-[200px]">
                      {category.description || "Segment lacks strategy overview. Update required."}
                    </p>
                  </div>
